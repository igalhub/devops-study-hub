...
export default function RecentDropdown() {
  ...
  const [recent, setRecent] = useState(() => getRecentLessons())
  const ref = useRef(null)

  useEffect(() => {
    const handler = () => setRecent(getRecentLessons())
    window.addEventListener('recent-updated', handler)
    return () => window.removeEventListener('recent-updated', handler)
  }, [])
